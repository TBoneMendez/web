...
def expand_to_daily(tx_df: pd.DataFrame) -> pd.DataFrame:
    def _expand(g: pd.DataFrame) -> pd.DataFrame:
        ...
        # payoff date: actual if repaid, else estimate
        if is_repaid:
            payoff = g.loc[g["transaction_norm"] == "principal_repaid", "date"].max()
        else:
            first_int = g.loc[g["transaction_norm"].isin(["interest", "interest_penalty"]), "date"].min()
            payoff = (first_int + relativedelta(months=int(meta["duration_months"] - 1))) if pd.notna(first_int) else est_end
        daily["estimated_tilbakebetalt_date"] = payoff
...
def build_views(daily_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    by_loan = (daily_df.groupby(["loan_id", "Company", "duration", "interest"], as_index=False)
        .agg(
            ...
            estimated_tilbakebetalt_date=("estimated_tilbakebetalt_date", "max"),
